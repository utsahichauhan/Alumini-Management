...
Class Action {
	...
	function delete_user(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM logininfo_users where id = ".$id);
		if($delete)
			return 1;
	}
	function signup(){
	...
	function update_account(){
		...
		if(!empty($password))
		$data .= ", password = '".md5($password)."' ";
		$chk = $this->db->query("SELECT * FROM logininfo_users where email = '$email' and id != '{$_SESSION['login_id']}' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("UPDATE logininfo_users set $data where id = '{$_SESSION['login_id']}' ");
		if($save){
			$data = '';
			foreach($_POST as $k => $v){
				if($k =='password')
	...
	function update_alumni_acc(){
		extract($_POST);
		$update = $this->db->query("UPDATE logininfo_users set status = $status where id = $id");
		if($update)
			return 1;
	}
	...
	function save_event(){
		...
		if($_FILES['banner']['tmp_name'] != ''){
						...
						$move = move_uploaded_file($_FILES['banner']['tmp_name'],'images/'. $fname);
					$data .= ", banner = '$fname' ";
